...
namespace Game.Map
{
    ...
    {
        [Header("画布引用")] public RawImage drawBoardUI;
        public RenderTexture mapCanvasRT;

        [Header("缩放设置")] public Slider zoomSlider;
        ...
        [Header("平移设置")] private float currentZoom;
        ...
        {
            bool isMouseInBoard =
                RectTransformUtility.RectangleContainsScreenPoint(drawBoardUI.rectTransform, Input.mousePosition);
        ...
        public Vector2 GetMousePosInCanvas()
        {
            ...
            Vector2 localPoint;
            RectTransformUtility.ScreenPointToLocalPointInRectangle(drawBoardUI.rectTransform, Input.mousePosition,
                null, out localPoint);
            float x = Mathf.Clamp((localPoint.x + rect.width / 2) / rect.width * mapCanvasRT.width, 0,
                mapCanvasRT.width - 1);
            float y = Mathf.Clamp((localPoint.y + rect.height / 2) / rect.height * mapCanvasRT.height, 0,
                mapCanvasRT.height - 1);
